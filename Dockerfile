...
RUN curl -o /usr/local/src/luajit.tar.gz -SL http://luajit.org/download/LuaJIT-2.0.4.tar.gz \
...
RUN curl -o /usr/local/src/lua-nginx.tar.gz -SL https://github.com/openresty/lua-nginx-module/archive/v0.10.6.tar.gz \
...
RUN curl -o /usr/local/src/nginx.tar.gz -SL https://nginx.org/download/nginx-1.10.1.tar.gz \
	...
	&& ./configure --prefix=/opt/nginx --with-ld-opt="-Wl,-rpath,/usr/local/luajit/lib" --add-module=../ngx_devel_kit-0.3.0 --add-module=../lua-nginx-module-0.10.6 \
	&& make -j2 \
	&& make install
